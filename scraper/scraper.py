...
class WebScraper:
    ...
    async def write_rows(self, url: str):
        words_list = await self.get_words_list(url)
        async with aiofiles.open(
            self.CSV_PATH, mode="a", encoding="utf-8", newline=""
        ) as afp:
            writer = AsyncWriter(afp, dialect="unix")
            async for row in self.generate_row(words_list):
        ...
        url = await self.get_next_url(url)
    ...
    async def generate_row(
        ...
        for word_el in words_list:
            ...
            soup = await self.make_soup(word_url)
    ...
    async def get_url_response(self, url: str):
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as resp:
                return await resp.text()

    async def get_next_url(self, url: str) -> str:
        soup = await self.make_soup(url)
        try:
    ...
    async def make_soup(self, url) -> BeautifulSoup:
        if not (response := await self.get_url_response(url)):
    ...
    async def get_words_list(self, url: str) -> Optional[List[BeautifulSoup]]:
        soup = await self.make_soup(url)
